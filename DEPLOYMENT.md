# Deployment Guide - IchanHub

Panduan ini menjelaskan langkah-langkah men-deploy aplikasi **IchanHub** (Native PHP) ke layanan Shared Hosting standar (berbasis cPanel).
...
1.  **Buat Database Baru:**
    ...
    *   Buat database baru, misalnya: `u12345_ichanhub`.
    *   Buat user database baru, misalnya: `u12345_ichan_admin`.
...
2.  **Import Struktur Database:**
    ...
    *   Pilih database `u12345_ichanhub` di sidebar kiri.
    *   Klik tab **Import**.
...
class Database {
    ...
    private $db_name = "u12345_ichanhub"; // Nama DB Hosting
    private $username = "u12345_ichan_admin"; // User DB Hosting
...
3.  (Opsional) Jika ingin diakses via `domain.com/ichanhub`, buat folder `ichanhub` di dalam `public_html`.
...
2.  **Redirect ke Halaman Utama:**
    *   Secara default, pengunjung harus mengetik `domain.com/ichanhub/src/html/index.html`.
    *   Untuk mempermudah, buat file `index.php` di root folder (`public_html/ichanhub/`) dengan isi:
    ```php
    <?php
...
1.  Buka browser dan akses URL aplikasi Anda (misal: `http://namadomain.com/ichanhub`).
